...
An \code{\#ORIGIN} marker tells \code{gencot-mrgcomments} to prepend to the following line the before-unit of original 
source line \code{<bline>}. If \code{<aline>} is present it tells to append to the following line the after-unit of original 
source line \code{<aline>}. An \code{\#ENDORIG} marker tells \code{gencot-mrgcomments} to append to the \textit{previous}
line the after-unit of original source line \code{<aline>}. Thus, by surrounding a target code part with an \code{\#ORIGIN} and
\code{\#ENDORIG} marker the comments are placed as intended.

Several markers affect the same line, if they are placed in lines immediately following each other. All such \code{\#ORIGIN}
markers affect the next following line without a marker, all such \code{\#ENDORIG} markers affect the previous line without
a marker. If several \code{\#ORIGIN} markers affect the same line, \code{<aline>} specifications are ignored in all but
the last marker. An \code{\#ORIGIN} marker line must not be followed immediately by an \code{\#ENDORIG} marker line.

If a line is affected by several markers, the corresponding comment units are concatenated in the order of the markers.
Before-units are concatenated without any separator. After-units are separated by a newline to end possibly trailing
line comments.
...
an END rule. The BEGIN rule reads the <file> line by line and collects before- and after-units as strings in the arrays \code{before} and \code{after}. The arrays are indexed with the line number of the separator between before- and after-unit.
