...
compilation unit, it must also be translated to Cogent so that it is known in the Cogent propgram. Functions are translated
...
by the Cogent compilation unit.
...
It is intended as a filter for the processors \code{gencot-externs}, \code{gencot-exttypes}, \code{gencot-dvdtypes},
...
A toplevel item is a tagged struct, union, or enum, a typedef name, a function or a global variable. 
...
and typedef names defined in system include files, \code{items-used} starts with all initial items belonging
...
To further reduce external items, the types of external items are ``fully resolved'', i.e.~all
...
Finally, the tags and typedef names are reduced to those defined in a system include file using the same
heuristics as above. Together with the names of the external invoked functions and variables their items
...
The processor translates these declarations using the function \code{transGlobals} of
...
The task of \code{gencot-exttypes} is to translate all tag and typedef name definitions listed in the 
file of used external items. 
it parses and analyses
all C source files specified in the input file name list and combines the resulting symbol tables as
described in Section~\ref{impl-ccode-package}. The tag and typedef name definitions in the resulting
table listed in the argument file are then translated.
...
All types used in these definitions are fully resolved, as described in Section~\ref{impl-ccomps-items}.
...
\end{verbatim}
in module \code{Gencot.Cogent.Translate} where the string list contains the directly referenced type names.
...
although array types with a specific size may be associated with several different items.
...
For fully resolved function types an additional type synonym definition must generated for the translated function type.
...
a derived function, array, or pointer type or a tagless struct or union type, its sub-items according to the 
type are processed recursively. For a tagged composite type item its members are processed recursively as sub-items.

An item is processed as follows. If its declared type is primitive or an enum or a function pointer no output is generated 
since properties are not 
...
description. The filter processes every type name definition in the source file where the defining type
...
intended to be converted to property declarations for being used by \code{gencot-translate}
...
\textbf{deprecated, the generated list is now produced by \code{items-used}}.

The processor \code{parmod-externs} is used to generate the parameter modification descriptions intended
to be converted to item property declarations used by \code{gencot-externs} (see Section~\ref{impl-ccomps-externs}) 
and \code{gencot-exttypes} (see Section~\ref{impl-ccomps-exttypes}). It is invoked as
\begin{verbatim}
  parmod-externs <used external items file>
\end{verbatim}
where the argument is the list of external toplevel items used by the Cogent compilation unit, as it is created 
by \code{items-used}.
