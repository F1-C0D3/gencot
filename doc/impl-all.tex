...
Additionally, if the unit name is \code{<uname>} they use the auxiliary files
\begin{description}
...
be listed, include files \code{x.h} must not be listed. The commands process all files listed in the unit file
together with all included files.
\item[\code{<uname>.unit-manitems}] Manually specified external items, as described in Section~\ref{impl-ccomps-items} as
...
The \code{auxcog} command only uses the unit file \code{<uname>.unit} as auxiliary file.
