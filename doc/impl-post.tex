...
Occurrences of \code{NULL} must be treated in a special way. They are syntactically translated by Gencot to a reference to a
...
\subsubsection{NULL Pointer Processing}
...
described above. If the variable \code{v} has not-null type it can be immediately substituted by \code{True} or \code{False}. Otherwise
it may be used to transform the scope of the variable \code{v} by duplicating it and changing the type of \code{v} to not-null in one copy
and replacing it by \code{null/roNull} in the other copy. If the NULL test has been used in C to distinguish between alternative code
branches, every copy can be reduced to the corresponding branch.

This approach only produces useful code, if the scope of \code{v} is determined in a useful way. If it is too large, code is unnecessarily
duplicated. If it is too small, the NULL test has not enough effect. In particular, if \code{v} is a value variable, the use in the NULL
test is the only reference to it and the remaining scope is empty. Gencot uses a heuristics to determine the scope affected by NULL tests.
It works by moving the NULL test in the code to the position where the scope should begin and by nesting \code{let} expressions to determine
the end of the scope.

To be able to move the NULL test in the code it is always converted to a binding of the form \code{v<n>' = v op cogent\_NULL}. If it does
not have this form a new value variable \code{v<n>'} is introduced and the original NULL test is replaced by it. This means that the
postprocessing step may have to introduce additional value variables. To simplify this Gencot uses variables of the form \code{vv<n>'}
where \code{<n>} is a unique number starting at \code{0} in every function body expression. For simplicity, the following description
always uses normal value variables of the form \code{v<n>'}. As a binding the NULL test may be moved freely in the code, as long as the
single occurrence of \code{v<n>'} remains in its scope and the test remains in the scope of the binding of the tested variable \code{v}.

If the variable \code{v} is a value variable, the NULL test





tested for being \code{NULL} using the Gencot basic operations \code{notNull},
\code{roNotNull}, and \code{isNull} defined in Section~\ref{design-operations-null}.
