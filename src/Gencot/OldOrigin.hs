...
-- Origins are always created in a way that either both lists are empty or both are nonempty
isOrigin :: Origin -> Bool
isOrigin Origin [] [] = False
isOrigin _ = True
...
listOrigin f cs = 
    if null ocs then noOrigin
                else Origin [((fst.head.sOfOrig.head) ocs, True)] [((fst.head.eOfOrig.last) ocs, True)
    where ocs = filter isOrigin $ map f cs
