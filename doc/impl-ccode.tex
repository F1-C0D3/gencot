...
(see Section~\ref{imppl-ccode-expr}). Note that no \code{Origin} components are added, since the types \code{Exp} and 
...
\subsection{Repeated Origin Markers}
\label{impl-ccode-reporigs}

The filter \code{gencot-reporigs} for removing repeated origin markers is implemented as an awk script.
