...
operator \code{*}, a function invocation which returns the pointer or container as its result, a conditional expression, or
a comma expression. A container can also be designated by a compound literal. A pointer can also
...
which is bound to \code{v<n>'}. The mapped identifier is then used as side effect target. If there is no sharing, all
...
translation stages. If the variable is determined to be the error variable \code{err'} because the container has not been
specified in C by a variable or a chain of nested accesses to a variable the \code{put} operation is replaced by a unit
binding \code{() = ()} to prevent a free occurrence of the error variable in the \code{put} expression.

If the container is a struct which does not contain (directly or indirectly) a pointer it may be discarded in Cogent
after accessing a component. Therefore a component access where the container is specified by the result of a function invocation,
a conditional expression, a comma expression, or a compound literal can be represented in Cogent as long as the component
is not modified, so that no \code{put} operation is required. However, in the first three cases the container is no lvalue
expression in C and cannot be modified. A compound literal is an lvalue expression and can be modified, but compound literals
are not (yet) supported by Gencot. Gencot translates these access expressions in the same way as all others, using a \code{take}
operation where the remaining container is specified by the error variable and the \code{put} operation is replaced by
a unit binding. The \code{take} operation will be replaced by a valid member access in later processing steps.

If the container is an array, the \code{put} operation needs to use the same element index as the \code{take} operation. 
In the \code{take} operation it is available as a value variable. Since value variables are not reused in the body of a
function, the same value variable is used in the \code{put} operation.
...
A struct member access \code{e.m} has only one subexpression \code{e} for denoting the struct. If it is a variable or an access chain
starting with a variable a read access is translated to the following binding sequence:
\begin{verbatim}
  <v>{m=p<k>'} = v<m>'
  v<n>' = p<k>'
  <v> = <v>{m=p<k>'}
\end{verbatim}
where \code{v<m>'} is the variable to which the value of \code{e} has been bound by the translation, \code{<v>} is a variable
depending on the expression bound to \code{v<m>'} in translation of \code{e} as described above, and \code{p<k>'}
is a new component variable. The member value is available as \code{v<n>'}.
...
The array access is translated by first concatenating the binding list for \code{e1} after \code{e2}. Then, if \code{e1} is a variable
or an access chain starting with a variable a read access is translated to the following binding sequence:
\begin{verbatim}
  <v> @{@v<l>'=p<k>'} = v<m>'
  v<n>' = p<k>'
  <v> = <v> @{@v<l>'=p<k>'}
\end{verbatim}
where \code{v<m>'} is the value variable bound in the translation of \code{e1}, \code{v<l>'} is the value variable bound in the translation
of \code{e2}, and \code{<v>} and \code{p<k>'} are as above. The element value is available as \code{v<n>'}.

If the array element is used as an lvalue the component variable \code{p<k>'} is re-bound to the new value by additional
bindings inserted between the third and fourth binding.

Here the specific Cogent array \code{take} and \code{put} operations are used. In Cogent they can only be applied if \code{v<m>'}
...
to any pointer which is not a function pointer, therefore the type of \code{v<m>'} can result from mapping any C array or pointer
...
postprocessing steps, depending on the type of \code{v<m>'}.
...
\end{verbatim}
where \code{v<m>'} is the value variable bound in the translation of \code{e}. The types \code{ft} and \code{fpt} are
...
operation as described for struct member accesses. If \code{e} is a variable or an access chain
starting with a variable a read access is translated to the following binding sequence:
\begin{verbatim}
  <v>{cont=p<k>'} = v<m>'
  v<n>' = p<k>'
  <v> = <v>{cont=p<k>'}
\end{verbatim}
where \code{v<m>'} is the value variable bound by the translation of \code{e}, \code{<v>} is a variable
depending on the expression bound to \code{v<n>'} in the translation of \code{e} as described above, and \code{p<k>'}
is a new component variable. The referenced value is available as \code{v<n>'}.
...
\begin{verbatim}
  ...
  gsp = gsp{cont=p<k>'}
\end{verbatim}
where \code{gsp} is the name of the parameter with the Global-State property and \code{v<m>'} is the value variable bound
to it and \code{p<k>'} is a new component variable. The value of the global object is available as \code{v<n>'}.
...
\begin{verbatim}
  v<n>' = v<n0>' (v<n1>',..,v<nm>')
\end{verbatim}
where \code{v<n0>'} is the value variable bound in the translation of the function subexpression \code{f}.
...
Heap-Use, Input-Output, Add-Result, and Global-State. The other item properties only affect the translation of types and are thus implicitly
...
The item properties Heap-Use, Input-Output, and Global-State introduce additional function arguments, for them actual values
must be determined to pass to the function invocation. In all three cases the additional arguments must also be present for
the surrounding function definition for which the body contains the invocation. The corresponding variable names are used as
actual arguments for the invocation. In the case of Heap-Use and Input-Output it is a fixed name as described in
Section~\ref{design-types-itemprops}, in the case of Global-State it is the name of the corresponding virtual parameter.

The item properties Add-Result, Heap-Use, Input-Output, and Global-State introduce additional result components. In that case
the binding has the form
\begin{verbatim}
  (v<n>',v1..) = v<n0>' (v<n1>',..,v<nm>')
\end{verbatim}
with a tuple pattern. The variables \code{v1..} correspond to the properties Add-Result, Global-State, Heap-Use, Input-Output in that order.
...
added by a Global-State, Heap-Use, or Input-Output property is the same which is used as actual argument for the corresponding parameter, i.e.,
...
Gencot translates lvalue subexpressions in the same way as other subexpressions to a binding list using \code{bindExpr}.
...
\begin{verbatim}
  v<m>' = v
\end{verbatim}
which makes the value of \code{v} available for read accesses using the value variable \code{v<m>'}. If the expression is used
...
It is also the result of the assignment expression, therefore it is additionally bound to the value variable \code{v<n>'}.
Note that this is the only case where a value variable can be used twice, however, it is always used in the same binding.

Of course, appending this binding makes the binding for the read access obsolete, because \code{v<m>'} is never
...
\begin{verbatim}
  (v2',x) = (v1',v1')
\end{verbatim}
so that \code{v2'} represents the result value of the assignment expression and \code{x} is the modified side effect target.
...
for the rhs. Let \code{v<j>'} be the value variable bound in the translation of \code{e1} and \code{v<k>'} be that for \code{e2}.
...
For an increment/decrement operator such as \code{e++} there is only one subexpression \code{e}. Let \code{v<j>'} be the value
...
A conditional expression \code{e0 ? e1 : e2} is translated to a binding list as follows. First the three subexpressions are translated
to binding lists \code{bs0}, \code{bs1}, and \code{bs2} by \code{bindExpr}. Let \code{v<j>'}, \code{v<m>'}, and \code{v<k>'} be the
value variable to which the result of \code{e0}, \code{e1}, \code{e2}, respectively, is bound in the translation. Let \code{x1..}
and \code{y1..} are the side effect targets in the translations of \code{e1} and \code{e2}, respectively, and let \code{z1..} be
the union of both in some order. The binding list of the translated expression is the list of \code{e0} to which the binding
\begin{verbatim}
  (v<n>',z1..) = 
  if v<j>'
...
\end{verbatim}
is appended. According to the type of \code{v<j>'} the condition in the conditional expression is adjusted in later stages.
...
evaluated or not. It would be possible to appended the lists \code{bs1} and \code{bs2} to the list of \code{e0} outside of the
conditional expression and replace the \code{let} expression in the corresponding condition branch by the tuple \code{(v<m>',z1..)} or \code{(v<k>',z1..)}, respectively. However, then implications of the condition cannot be used in \code{bs1} and \code{bs2}.
At least NULL test conditions are exploited during postprocessing to change the type of tested values from \code{MayNull} wrapped
to unwrapped. To make this possible the lists \code{bs1} and \code{bs2} are always placed in the condition branches, as shown above.
...
\end{verbatim}
which will be simplified to \code{(v4',v) = if x then (i/=0,i) else (False,v)} in later stages,
...
\end{verbatim}
which will be simplified to \code{(v = i) and (v4' = if i/=0 then x else False)} in later stages.
...
are respected by the translation, the Cogent type includes all additional arguments (for Global-State, Heap-Use, and Input-Output
properties) and all additional result components (for Global-State, Heap-Use, Input-Output, and Add-Result properties).
...
An expression statement has the form \code{e;}. It is translated by translating \code{e} to an \code{ExprBinds} with
a binding list \code{bs}. Then the translation of the expression statement is the binding
\begin{verbatim}
  (c',v1..) = let bs in (0,v1..)
\end{verbatim}
where \code{v1..} are the side effect targets of \code{bs}. This discards the value \code{v<n>'} of \code{e}.
...
For a \code{return} statement with an expression the expression is translated to an \code{ExprBinds} by \code{bindExpr} with
a binding list \code{bs}. Then the translation of the return statement is the binding
\begin{verbatim}
  (c',r',v1..) = let bs in (3, v<n>',v1..)
\end{verbatim}
where \code{v<n>'} is the value variable bound in \code{bs} to the result of the expression. This binding explicitly
binds the result variable \code{r'} as additional side effect target. The type of \code{r'} is always
...
A declarator is translated to a pair consisting of a variable name and an \code{ExprBinds}. The variable name is the declared identifier
mapped to a Cogent name, as described in Section~\ref{impl-ccode-names}. If the declarator has an initializer the \code{ExprBinds}
results from its translation. Otherwise an \code{ExprBinds} with the single binding \code{v' = defaultVal ()} is used.
...
As described above a pair consists of a variable \code{v} which corresponds to the declared identifier and an \code{ExprBinds}
with a binding list \code{bs} which binds the value of the initializer or the default value to \code{v<n>'} and has the
side effect targets \code{v1..}. It is combined with a binding \code{(c',y1..) = expr2} as follows.
...
Add-Result, Global-State, Heap-Use, and Input-Output.

In the special case of the function named \code{main} in C the default result \code{0} is added by prepending a binding for the
result variable \code{r'}:
\begin{verbatim}
  let r' = 0
  and (c',v1..) = expr
  in <result expression>
\end{verbatim}
so that \code{r'} is bound even if the function does not contain \code{return} statements.
...
\begin{verbatim}
  (r', v1..)
\end{verbatim}
respectively, where the variables \code{v1..} correspond to the properties Add-Result, Global-State, Heap-Use, and
Input-Output in that order.
...
First the subexpression \code{e} is translated to an \code{ExprBinds} with a binding list \code{bs} which binds
the value of \code{e} to \code{v<n>'} and has the side effect targets \code{v1..}.
The substatements are translated to bindings \code{(c',x1..) = expr1} and \code{(c',y1..) = expr2}.
...
If a branch in a selection statement uses a \code{return} statement its translation binds the result variable \code{r'}. Then all
other branches must also bind \code{r'} so that the selection statement can bind it as a whole. If another branch does not use
a \code{return} statement (atleast not in all its sub-cases) this will result in a reference to an unbound instance of \code{r'}.
This will be detected statically by the Cogent compiler and signalled as an error.

It would be possible to avoid this by binding \code{r'} to a default value at the beginning of each translated function. However,
the Gencot \code{default} operation (see Section~\ref{design-operations-default}) does not support default values for arbitrary
result types. Therefore Gencot does not handle this case, it must be handled by modifying the C program so that for every selection
statement either all branches are left by a \code{return} statement or none. Note that this is not necessary for \code{break}
and \code{continue} statements.

In C this situation is often handled by sequentially executing a \code{return} statement after the selection statement which will
handle all branches not left by a \code{return} statement. Then the C program can be modified by moving this \code{return} statement
into all branches. A specific situation is a \code{switch} statement without a default case, because it always has an ``invisible''
empty default case. If all explicit cases are left by a \code{return}, there is still the implicit default case for which this
is not the case. Thus it must be changed by adding an explicit default case which is also left by a \code{return} statement.
...
and where \code{e2} has no side effects is as follows. The statement \code{if e2 then s else break;} is translated to a binding
\code{(c',x1..) = expr} by applying \code{bindStat} to it. The expression
\code{e3} is translated to a binding list \code{bs3} by applying \code{bindExpr} to it. Conceptually, \code{e3} is appended
as an expression statement to \code{s}, by combining
all bindings to the binding \code{(c',y1..) = exprstep} defined by
\begin{verbatim}
  (c',y1..) = let (c',x1..) = expr
  in if c' > 1
     then (c',y1..)
          ...
          in (0,y1..)
\end{verbatim}
where \code{y1..} is the union of the side effect targets in \code{bs3} and \code{x1..}. Note that the condition uses \code{c' > 1}
...
\end{verbatim}
where \code{w1..} are all variables occurring free in \code{exprstep} which are not part of \code{y1..}, in some order.
...
above for a block item sequence. If \code{c1} is an expression it is translated to a binding list \code{bs1} by
applying \code{bindExpr} to it. Then the translation of the \code{for} statement is the binding
\begin{verbatim}
  (c',z1..) = 
  let bs1
  and (c',y1..) = exprloop in (c',z1..)
\end{verbatim}
where \code{z1..} is the union of \code{y1..} and the side effect targets in \code{bs1} in some order. This corresponds to a
simplified sequence of \code{bs1} and the \code{exprloop} binding exploiting the fact that the first results from an expression
and can therefore not contain a jump.
...
If the loop satisfies these properties the resulting C expression \code{emax} is translated to a binding list \code{bs} \code{(v<n>',v1..) = expr}
by applying \code{bindExpr} to it. Then the expression \code{exprmax} is
\begin{verbatim}
  let bs in v<n>'
\end{verbatim}
where \code{v<n>’} is the value variable bound in bs to the result of the expression.
Note that side effect targets could occur in \code{bs} if \code{emax} is \code{ini}. In this case the side effects are already
