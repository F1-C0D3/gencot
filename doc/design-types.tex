...
Gencot by encoding the function type as an abstract type name (see Section~\ref{design-names}). 
...
result type of a function pointer type, Gencot implements the encoding for all mapped C types.

If the base type of a function pointer type is specified as a typename in C, its encoding is the mapping of 
this typename, which is a non-abstract type in Cogent. To also hide the function type in this case a pseudo 
derivation step of the form \code{FNN\_} is prepended to the mapped funtion type name.
