...
As irrefutable pattern always a tuple of vaiables is used, it has the form
\begin{verbatim}
  PTuple [PVar v1, ..., PVar vn]
\end{verbatim}
where the \code{vi} are string constants for the variable names. For simplicity we use this form also if 
\code{n} is 1, this case is simplified to the form \code{PVar v1} only in the final step.

\subsubsection{Translating C Expressions}

Gencot uses a common monadic action 
\begin{verbatim}
  bindExpr :: CExpr -> FTrav GenBnd
\end{verbatim}
to translate all expressions, both with and without side effects. 

As variable for the expression value it always uses the variable named \code{v'}. Due to the prime character this is
a valid identifier in Cogent but not in C, so it cannot collide with any other variable taken and mapped from the C
code.

An expression \code{e} without side effect is translated to the binding
\begin{verbatim}
  (v') = expr
\end{verbatim}
where \code{expr} is the translation of \code{e} to a Cogent expression. If \code{e} has side effects the affected 
variables are added to the binding in arbitrary order as described in Section~\ref{design-cstats-expr}.

If Gencot cannot translate an expression the rhs of the binding is a dummy expression as described in 
Section~\ref{design-cstats-dummy}. The lhs may still contain additional variables, if Gencot detects that the expression
modifies these C variables.
