...
present in the Cogent types. Typedef names are usually mapped to Cogent type names. Therefore Gencot implements an alternative
mapping of types where all type names are fully resolved, and uses it here, so that the Cogent types added to the Cogent
AST are fully resolved and standalone.

The type information is added to the following nodes in the Cogent AST: \code{Pattern}, \code{IrrefutablePattern},
and \code{Expr}. It cannot be added to \code{Binding} and \code{Alt}, because these nodes cannot be extended. 
The remaining nodes already have enough type information available.
...
\begin{verbatim}
  ...
  data GenExpr =
    GenExpr (Expr GenType GenPatn GenIrrefPatn () GenExpr) Origin GenType (Maybe Stm)
  data GenPatn =
    GenPatn (Pattern GenIrrefPatn) Origin GenType
  data GenIrrefPatn = 
    GenIrrefPatn (IrrefutablePattern VarName GenIrrefPatn GenExpr) Origin GenType
  data GenType = 
    GenType (Type GenExpr () GenType) Origin
...
The third components in \code{GenExpr}, \code{GenPatn}, and \code{GenIrrefPatn} is the fully expanded Cogent type intended
for postprocessing, as described above.
...
\begin{verbatim}
  (v1..) = expr
\end{verbatim}
where the pattern is the unit pattern if there are no \code{v1..}.
...
operation as described for struct member accesses. Semantically this means that the referenced content is separated from the 
pointer in a \code{take} binding and later both are recombined by a \code{put} binding. This translation is modified in later stages.
...
bound to a value variable and that variable is used to specify the function on the rhs. If the function has result type 
...
it is the name of the corresponding parameter of the surrounding function. If the function has result type 
\code{void} in C the variable \code{v<n>'} is omitted, i.e. the binding is
\begin{verbatim}
  (v1..) = f(v<n1>',..,v<nm>')
\end{verbatim}
If there is only one result component the tuple pattern becomes a single variable.
...
\end{verbatim}
where \code{i} is the position of the parameter with the Modification-Function property.
...
Add-Result, Global-State, Heap-Use, and Modification-Function. 
...
If the C function result type is \code{void}, the component \code{r'} is omitted. If then there is only one variable left
the tuple pattern is replaced by a pattern for that variable.
