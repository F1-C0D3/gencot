...
\code{U32} and \code{Bool} can always be resolved by conversion. An expression \code{e} of type \code{U32} can be converted
to \code{Bool} by
\begin{verbatim}
  e /= 0
\end{verbatim}
and an expression \code{e} of type \code{Bool} can be converted to \code{U32} by
\begin{verbatim}
  if e then 1 else 0
\end{verbatim}
If the integer type is not \code{U32} additional conversion is required as described for type clashes among the integer types
below.
...
\code{e} is the translation of \code{expr}. These expressions will be further postprocessed as described below for pointers,
among others to evaluate it to \code{True} if the type of \code{e} is an unwrapped linear type, or resolve a clash between the
type of \code{e} and \code{MayNull CVoidPtr} otherwise.
...
Type clashes with \code{Bool} are resolved by converting the subexpression to the context type, if that is possible, otherwise
the subexpression is replaced by a dummy expression and an error is signaled. If the type clash is between two operator arguments
...
Postprocessing for boolean values is implemented by the monadic action
\begin{verbatim}
