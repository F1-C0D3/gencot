...
For arrays with nonlinear element type Gencot provides the polymorphic abstract functions
\begin{verbatim}
  arrGet : all(arr,idx,el:<DS). (arr!,idx) -> Option el
  arrSet : all(arr,idx,el:<DS). (arr,idx,el) -> arr
\end{verbatim}
with the expected semantics. The type variable \code{arr} denotes the array type, \code{idx} denotes the index type and
...
For every array type \code{A'<size>'\_El} used in the program, where \code{El} is nonlinear, Gencot automatically 
generates C implementations for instances of both functions which have the form
\begin{verbatim}
  arrGet[A'<size>'_El,UNN,El]
  arrSet[A'<size>'_El,UNN,El]
\end{verbatim}
and are implemented in C using the C array subscription operator \code{[]}. 
\code{UNN} is one of \code{U8, U16, U32} according to the \code{<size>} of the array. If the specified index 
is not in the range \code{0..<size>-1} the \code{arrGet}-instance returns \code{None}, the \code{arrSet}-instance
returns the unmodified array.

For arrays with linear element type Gencot provides the polymorphic abstract functions
\begin{verbatim}
  arrReplace : all(arr,idx,el). (arr,idx,el) -> (arr,el)
...
which is passed as the third parameter. The modification function may need additional information which must be passed to it. 
To support arbitrary types of additional information the type variable \code{arg} is used.
Note that the functions cause no sharing or discarding for the linear array element values.

For every array type \code{A'<size>'\_El} used in the program, where \code{El} is linear, Gencot automatically 
generates C implementations for instances of both functions which have the form
\begin{verbatim}
  arrReplace[A'<size>'_El,UNN,El]
  arrModify[A'<size>'_El,UNN,El]
\end{verbatim}
If the specified
...
For a readonly array it is possible to access elements directly, even if they are of linear type. Gencot generates
an implementation of the instance
\begin{verbatim}
  arrGet[A'<size>'_El,UNN,El!]
\end{verbatim}
