...
For every linear non-abstract type \code{T} (i.e., every record type) used in the Cogent program, Gencot automatically
generates an implementation for instances of both functions in antiquoted C. These instances have the form:
\begin{verbatim}
  create[T take (..)]
  dispose[T take (..)]
\end{verbatim}
The \code{create} instance only allocates space on the heap but does not initialize it, hence the result is 
represented in Cogent by a record with all fields taken. They can be filled using the Cogent put operation.
The \code{dispose} instance expects a record with all fields taken and simply frees the heap space. In particular,
this forces that all fields of linear type must have been taken and must be disposed separately.

The create/dispose instances for C pointer types are described in Section~\ref{design-types-pointer}.
...
is \code{\#UA'n'\_T}. Since Cogent translates take and put operations for a record field to an assignment,
...
value. Since in C a function cannot return an array, the type \code{A'n'\_T} is used as result type.
\item as type of a function parameter. In C this is ``adjusted'' to the pointer-to-element type. Since this
corresponds to type \code{A'n'\_T}, it is used as parameter type in Cogent. Alternatively the adjusted type
could be mapped, resulting in type \code{P\_T} or even \code{T}. The type \code{A'n'\_T} is preferred here
...
\code{\#UA'n'\_T} is used as type for the corresponding Cogent record field.
...
typedef name as an alias for the Cogent type \code{A'n'\_T}. When a typedef name for an array type is used where 
type \code{UA'n'\_T} would be required, the typedef name is fully resolved and the type \code{UA'n'\_T} is used
instead.
\end{itemize}
...
\begin{verbatim}
  create[TA'<size>'_El]
\end{verbatim}
for every type \code{A'<size>'\_El} used in the program. The instance is implemented by simply allocating the
...
Since the Cogent type \code{A'<size>'\_El} is linear, values of it may not be discared. Gencot generates an instance 
\begin{verbatim}
  dispose[TA'<size>'_El]
\end{verbatim}
for every type \code{A'<size>'\_El} used in the program, implemented by using the C standard function \code{free}.
...
\end{verbatim}
The type variable \code{arr} denotes the array type, \code{idx} denotes the index type and
\code{el} denotes the element type. The result type of \code{arrGet} uses the generic type \code{Option} defined in
the Cogent standard library.  Function \code{arrGet} expects a readonly array, so it may only be used in a context where
...
For every array type \code{A'<size>'\_El} used in the program, Gencot automatically 
...
\code{UNN} is one of \code{U8, U16, U32} according to the \code{<size>} of the array. If the specified index 
is not in the range \code{0..<size>-1} the instance returns \code{None}. F

For a multidimensional array type \code{A'<size>'UA'<size>'...\_El} the index type is a tuple \code{(UNN1,UNN2,...)}.

For an array type \code{A'<size>'U\_El} with an unboxed element type the 
instance of \code{arrGet} has the form
\begin{verbatim}
...
\end{verbatim}
Function \code{arrSet} is only defined for a discardable element type. It simply discards the old value of the indexed element.
Function \code{arrExchng}
...
Function \code{arrModify} modifies the element at the specified position in-place by applying a modification function,
...
To support arbitrary types of additional information the type variable \code{arg} is used.
...
For every array type \code{A'<size>'\_El} used in the program, and every type \code{Arg} for which
...
\end{verbatim}
If the specified
index is not in the range \code{0..<size>-1} the first function returns the same element which has been
passed as parameter, both functions return the unmodified array. 

For every array type \code{A'<size>'U\_El} the instances have the form
\begin{verbatim}
  arrExchng[A'<size>'U_El,UNN,#El]
  arrModify[A'<size>'U_El,UNN,El,Arg]
\end{verbatim}

C implementations for instances of \code{arrSet} are only generated if the element type is nonlinear. For an array type
\code{A'<size>'\_El} it has the form
\begin{verbatim}
  arrSet[A'<size>'_El,UNN,El]
\end{verbatim}
and for an array type \code{A'<size>'U\_El} it has the form
\begin{verbatim}
  arrSet[A'<size>'U_El,UNN,#El]
\end{verbatim}
If the specified index is not in the range \code{0..<size>-1} the instance returns the unmodified array.
...
For every array type \code{A'<size>'\_El} used in the program Gencot automatically generates a C implementation
for an instance of the form
\begin{verbatim}
  arrAccess[A'<size>'_El,UNN,P_El]
\end{verbatim}
and for every array type \code{A'<size>'U\_El} with an unboxed element type a C implementation for an
...
As described in Section~\ref{design-types-struct} Gencot provides instances of the polymorphic functions \code{create}
and \code{dispose} for creating and disposing
values for all boxed record types. In the case of a mapped pointer type \code{P\_T} they have the form
\begin{verbatim}
