...
main = do
    ...
    extToplvs <- runFTrav table ("", parmods) $ transExtGlobals unitTypeNames $ filter isExternDef typeCarriers
    {- generate abstract definitions for derived types in all type carriers -}
    derivedToplvs <- runFTrav table ("", parmods) $ genTypeDefs unitTypeNames $ typeCarriers
    {- Output -}
    print $ prettyTopLevels (derivedToplvs ++ extToplvs)
