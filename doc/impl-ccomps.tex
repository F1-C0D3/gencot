...
The entry wrapper generation is implemented by the monadic action \code{genEntries} in the module 
\code{Gencot.C.Generate}:
\begin{verbatim}
...
Origin markers are added by wrapping every entry wrapper according to the start and end position of the 
...
The processor \code{gencot-externsac} generates the wrapper implementations and the implementations for 
