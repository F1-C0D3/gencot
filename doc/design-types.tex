...
with the C array type \code{t[n]}. 
...
The Cogent builtin array type is still under development. Its compilation to C is not yet stable and the refinement proof has
not yet been covered. Therefore the current version of Gencot does not use it for
...
identifier, it means that the identifier names a (constant) value of type \code{t*}, since in expression C arrays are 
...
type \code{el} is linear. For an abstract parameterized type Cogent considers the unboxed type always as nonlinear,
...
Therefore the decision in early versions of Gencot is to use the second approach. For every array \code{<size>} used in the C 
...
\subsubsection{Cogent Builtin Array Type}

Here we document considerations for using the Cogent builtin array type for mapping C array types. They are based 
upon the Cogent builtin array type implementation as of June 2020.

The Cogent builtin array type has the form \code{El[n]} for the linear case and \code{El\#[n]} for the nonlinear
case. The nonlinear case can alternatively be written as \code{\#(El[n])}, i.e., by applying the unbox type
...
C type \code{el[n]}. An exception are function result types, where \code{El\#[n]} is also translated to 
\code{el*}. 

Note that this translation schema is wrong, since for unboxed array types used as function parameter or result
it translates them to pointer types. This has been posted to the Cogent github as Issue\#370.
...
For the Gencot type mapping the builtin array types can be used as follows.

Since the unbox operator is available for array types it is not necessary to use different Cogent types for the 
linear and the nonlinear case, C array types could normally be mapped to boxed array types, if they occur in 
the nonlinear form the unbox operator can be applied.

However, the empty-value type must be constructed in a different way, instead of the operator \code{take (..)} 
...
For both reasons Gencot should also wrap array types in Cogent records when builtin array types are used. 
The resulting form is
\begin{verbatim}
  type CArr<size> el = {arr: el#[<size>]}
\end{verbatim}
where the builtin array type is used instead of the abstract type \code{UArr<size> el}. This approach has the
following consequences:
\begin{itemize}
\item Only one Cogent type name is required for every array size.
\item Type \code{\#(CArr<size> el)} is now considered to be linear if the element type \code{el} is linear,
because in this case the Cogent type checker recognizes the builtin array type \code{el\#[<size>]} as linear.
\item No double wrapping is required, since Cogent translates take and put operations for unboxed arrays
in a record as loops for assigning the single elements instead of the array as a whole. Note, however, that 
an assignment of a wrapped array may be more efficient, but this should be a decision for the implementation
of the builtin array types.
...
\item Since the unboxed array types are only used for record fields the error according to Issue\#370 is not 
relevant.
\end{itemize}

Multidimensional C array types could now be translated using a multidimensional Cogent array type, such as 
\code{el\#[<s1>]\#[<s2>]} (note that all dimensions must be unboxed). Cogent translates such types correctly back
to a multidimensional C array type. However, this form does not correspond to the form \code{CArr<s1> \#(CArr<s2> el)}
where both dimensions are wrapped. For consistency reasons the latter form is used by Gencot.

If the \code{<size>} is specified by a preprocessor identifier there is another subtle difference to the approach 
without builtin array types: in that approach the size information is only encoded in the abstract type name, 
there the identifier is used. To access the actual size value Genops needs the preprocessor definition of that
identifier. When using builtin array types the identifier is resolved by Cogent when it translates the builtin
array type and the actual size value is specified in the C array type where it can directly be retrieved by Genops.

For all these reasons Gencot switched to use Cogent builtin array types as described here for mapping C array types, 
as soon as the builtin array types have been available and sufficiently stable in Cogent.
