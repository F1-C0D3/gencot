...
If the binding lists result from translating a C statement (see below), the control variable \code{c'} occurs instead of the value 
variable \code{v<n>'}. 
...
be value variables since they may be used more than once. Gencot uses variable names of the form \code{r<n>'} for this 
...
A struct member access \code{e.m} has only one subexpression \code{e} for denoting the struct. If the last binding in its 
main list has the form
\begin{verbatim}
  (v<n>',v1..) = (expr,v1..)
\end{verbatim}
it is replaced by the bindings
\begin{verbatim}
  (<v>{m=r<k>'},v1..) = (expr,v1..)
  (v<n>',v1..) = (r<k>',v1..)
\end{verbatim}
where \code{<v>} is a variable or \code{\_} depending on \code{expr} as described above and \code{r<k>'} is a new component
variable. If \code{<v>} is not \code{\_} the binding
\begin{verbatim}
  <v> = <v>{m=r<n>'}
\end{verbatim}
...
The array access is translated by first concatenating the main binding list for \code{e1} after \code{e2}. If the last binding in this
list has the form
\begin{verbatim}
  (v<n>',v1..) = (expr,v1..)
\end{verbatim}
...
\begin{verbatim}
  <v> = <v> @{@i<k>'=r<k>'}
\end{verbatim}
is prepended to the putback list, where \code{<v>}, \code{r<k>'} are as above, \code{v<l>'} is the value variable used for
binding the value of \code{e2}, and \code{i<k>'} is the index variable corresponding to \code{r<k>'}.
...
in the translated expression its main list is concatenated after the main lists of all other subexpressions, then the last 
binding of this list is replaced by a binding
\begin{verbatim}
  (v<n>',v,v1..) = (expr,expr,v1..)
\end{verbatim}
where \code{expr} is the new value assigned as side effect by the translated expression (and also its result, therefore it is 
additionally bound to the (reused) value variable \code{v<n>'}. If \code{v} is a side effect target it 
is omitted from \code{v1..}. Note that this is the only case where a value variable can be used twice, if \code{expr} is a 
value variable, however, it is always used in the same binding.
...
of \code{e0}. Let \code{z1..} be the union of the side effect targets \code{v1..}, \code{x1..}, and \code{y1..} in some order. The main list 
...
\end{verbatim}
which will be simplified to \code{(v1',v) = if x then (i,i) else (0,v)} in later stages,
...
\begin{verbatim}
  ...
  v4' = 0
  v1' = 
    if v1' then v3' else v4'
\end{verbatim}
which will be simplified to \code{(v = i) and (v1' = i \&\& x)} in later stages.
...
translation to Cogent expressions before they are used as part of a translated statement. This would mean that taken
container components are put back locally in every translated expression. To support reuse of taken components the 
Gencot implementation postpones their putback in some situations. As in expression translation this is possible if
there are no conditional evaluations in a statement. Conditional evaluation is caused by conditional statements, switch
statements, loops and jump statements, but a sequence of simple statements involves no conditional evaluation (if not 
caused by conditional expressions or sequential logical operators) and can benefit from postponed putback.

To support postponed putback Gencot also translates C statements to a pair of binding lists, consisting of a main list
and a separate putback list. Note that putback lists are constructed in a way that they never access value variables.
Therefore it is possible to collect putback lists from several statements, although the value variables are reused
in every statement. Since component variables are accessed in putback lists, Gencot never reuses component variables
in different statements of the same function body.

Gencot uses a common monadic action 
\begin{verbatim}
  bindStat :: CStat -> FTrav ([GenBnd],[GenBnd])
\end{verbatim}
to translate every statement to a pair consisting of the main binding list and the putback list.

The last binding in the main list of a translated statement always has the form
\begin{verbatim}
  (c',v1..) = expr
\end{verbatim}
...
If Gencot cannot translate a statement it translates it to a single binding where the rhs is a dummy expression as described in 
...
An expression statement has the form \code{e;}. Its translation is that of \code{e}, with the binding
\begin{verbatim}
  c' = (False,False,None)
\end{verbatim}
appended to its main list.

A null statement has the form of a single semicolon \code{;} and is translated to a main list with the single
binding
\begin{verbatim}
  c' = (False,False,None)
\end{verbatim}
and an empty putback list.
...
A local declaration is a declaration which occurs as syntactical part of a compound statement. Gencot only translates 
local object declarations, it does not translate local type definitions and static assertions.

An object declaration introduces one or more names for C objects. The names shadow existing names in their scope, which
starts at the declaration and ends at the end of the next surrounding compound statement. In Section~\ref{design-cstats-general}
object declarations have been reduced to statements and no specific treatment was necessary. The reason is that there
every scope of a declared local name was translated to an own Cogent expression which automatically is the scope for 
variables bound in it. 

Due to postponed putback in the Gencot translation described here a shadowed name may be used after the scope of
a local name, therefore both must be separated. This is done by translating the scope of a local object declaration
always to a separate Cogent expression so that all putbacks for \code{take} operations are in the same scope.

Since the scope of a local declaration consists of the statements after it in the surrounding compound statement this is
an issue for the translation of compound statements. Otherwise, a local object declaration is translated to a pair
of binding lists in the same way as a statement, with two exceptions. First, the main list is extended by a binding
for exactly the variables introduced by the object declaration (either to the value of a translated initialzer or
to a default value, as described in Section~\ref{design-cstats-general}. No control variable is bound, since a declaration
cannot contain (jump) statements. The rest of the main list consists, as usual, of the sequenced main list of all 
subexpressions occurring in the initializers. Second, the initializers for struct and array
types cannot occur in a statement, Gencot provides special translation for them as part of the translation of local declarations.

\subsubsection{Compound Statements}

Compound statements are conceptionally grouped right-associatively. Thus a compound statement is either empty, or a single statement
or declaration (at the end), or it is a statement or declaration followed by a compound statement.

An empty compound statement is translated like a null statement to a main list with the single
binding
\begin{verbatim}
  c' = (False,False,None)
\end{verbatim}
...
For a single statement or declaration the translation of the compound statement is identical to that of the statement or
declaration.

A statement \code{s1} followed by a compound statement \code{s2} is translated as follows.

** todo **

A declaration \code{d} followed by a compound statement \code{s2} is translated as follows.

** todo **

\subsubsection{Function Bodies}

A C function body is always a compound statement. It is translated by \code{bindStat} and combined to a single
binding by \code{cmbBinds} of the form
\begin{verbatim}
  (c',v1..) = expr
\end{verbatim}
