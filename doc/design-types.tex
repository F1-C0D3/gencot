...
\begin{verbatim}
  el[<size>] -> A<size>_El , #A<size>_El
  el[<complex size>] -> A_El, #A_El
  el[] -> A_El , #A_El
  el[*] -> A_El , #A_El
\end{verbatim}
...
mapping the pointer to a Cogent boxed record type. 

It is tempting to employ this support also for all other pointer types by
mapping the pointer type to a Cogent record with a single field of the type referenced by the pointer. This 
would normally be binary compatible in C because a struct with a single field of type T is represented in memory
in the same way as a value of type T. However, when the C implementation of an abstract function uses the
C address operator to retrieve a pointer to a struct member or array element, the C pointer to T must be
cast to the pointer to the struct with the single field. Such type casts are not supported by the 
Isabelle C parser and must be avoided in the C program resulting from translating the Cogent program. 

For this reason all C pointer types which are not pointers to a struct are mapped to abstract types in Cogent.
...
A pointer type \code{t*} to a struct is mapped to the corresponding boxed record type, 
...
or again a pointer type is mapped to an abstract type with a generated name as described in Section~\ref{design-names}. This
name is used as parameter type name in mapped function pointer 
