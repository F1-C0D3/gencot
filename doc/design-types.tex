...
the easiest way would be to use a generic abstract type with a function type as single type argument. 
This could be thought of as an ``annotated'' or ``wrapped'' function type. 
...
However, if a function pointer type occurs as type of a struct member, any direct or indirect reference of a
...
Cogent by encoding the function type as an abstract type name (see Section~\ref{design-names}). 

The type name contains names for all parameter types and the result type. The goal is to use the same name 
...
Gencot could not separately compile different C files. Since every mapped C type may occur as parameter or
result type of a function pointer type, Gencot implements the encoding for all mapped C types. As described
...
Gencot conceptually uses the following two generic abstract types for representing binary compatible C function pointers:
\begin{verbatim}
...
the function has no parameters using the keyword \code{void}. The type argument must be the encoding of a Cogent function type, 
...
is specified but no parameter types. Here only the result type is used as type argument, it may be the encoding of an arbitrary 
...
and \code{B} are abstract, the current Gencot version uses non-generic abstract types instead. They have the form
\begin{verbatim}
  type CFunPtr_...
  type CFunInc_...
\end{verbatim}
where the \code{...} is the encoding of the function or result type as defined above. 

Note, that if the base type of a function pointer type is specified as a typename in C, its encoding is the mapping of 
this typename, which is a non-abstract type in Cogent. To also hide the function type in this case a pseudo 
derivation step of the form \code{FNN\_} must be prepended to the mapped funtion type name. In the current Gencot
version this is not used since the construction of the non-generic type name already hides the function type name.
...
\begin{verbatim}
  ...
  t (*)() -> #CFunInc_ENCT
\end{verbatim}
where \code{ENCFT} is the encoding of the function type \code{t( ... )} 
and \code{ENCT} is the encoding of the result type \code{t}.

As additional convenience, for every complete function pointer type a Cogent type name of the form \code{CFun\_ENCFT}
is introduced for the base function type. This makes it easy to generate both types from \code{ENCFT} in a preprocessor
macro.
