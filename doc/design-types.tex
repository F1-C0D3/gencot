...
However, the current Gencot implementation cannot detect the case where a typedef name is indirectly defined for a type
translated to \code{String}. In this case the bang operator is applied to the translated typedef name. As example, given
the C definitions
\begin{verbatim}
...
\begin{verbatim}
  ...
  type Cogent_t2 = t1
\end{verbatim}
and the type name \code{t1} will be translated as \code{Cogent\_t1} whereas \code{t2} will be translated as 
\code{Cogent\_t2!}. Since the bang operator is ignored by Cogent and indirect type name definitions should occur seldom 
...
be a linear type or not. If an array type is mapped to a linear type, a \code{const} qualifier for the array type
...
\begin{verbatim}
  const int gvar[5];
\end{verbatim}
Gencot translates the variable to an access function returning a pointer. The \code{const} qualifier causes the 
...
accordingly. Again, the \code{const} qualifier causes the parameter type to be readonly:
\begin{verbatim}
  cogent_f: (Carr5 U32)! -> ()
\end{verbatim}

If an array type is used as base type of a derived pointer type the pointer type is mapped to a readonly type, as
described above, if its base type is \code{const} qualified. Here the C rule must be taken into account that an array
type is \code{const} qualified if its element type is \code{const} qualified. The same applies if an array type is used
...
hence a \code{const} qualifier is irrelevant. Since the current Cogent version has problems when bang and unbox operators
...
cannot be made readonly in the definition, even if in C the array type is \code{const} qualified in the definition.
Instead, the mapped typedef name must be banged whenever no unbox operator is applied. If the array type is not 
\code{const} qualified in the definition, the mapped typedef name must only be banged if the typedef name is \code{const}
