...
value as a pointer of the boxed type \code{CArr<size> El} which is never \code{NULL}. Since in Cogent boxed types
...
Since \code{NULL} may be passed as parameter value, the type is wrapped as \code{MayNull} (see~\ref{design-types-pointer}).
...
or it must be wrapped as \code{MayNull}.
\begin{verbatim}
