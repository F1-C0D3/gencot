...
\end{verbatim}
which binds the resulting symbol table to variable \code{table}. \code{runTrav\_}
returns a result of type \code{Either [CError] (DefTable, [CError])}, where \code{DefTable}
...
\code{Stm} already contain \code{Origin} information. The Cogent AST expression type is not used by Gencot. 
...
\code{Language.C.Analysis.TravMonad} and mainly provides the symbol table as global state during the traversal.
...
The monad provided by language-c is parameterized by a type for an additional ``user state''. Gencot uses this
state extension to make the actual name of the processed file available during processing. In the \code{NodeInfo}
values in the AST it is always specified as \code{<stdin>} since the input is read from a pipe. The monadic
action \code{srcFileName} returns the file name for a \code{NodeInfo} value and replaces it by the name in the user
state if it is equal to \code{<stdin>}.

The utilities for the monadic traversal are defined in module \code{Gencot.Traversal}. The monadic type is
defined as
\begin{verbatim}
  type FTrav = Trav String
\end{verbatim}
where \code{String} is the type used for the user state. As execution function for the monadic actions
the function
\begin{verbatim}
...
\code{IO} action and reads the actual file name from the command line arguments and uses it to
