...
\item[\code{gencot-parmodtmpl}] processes a single file \code{x.c} and generates the Json template for specifying
...
help of \code{getExtInvocations} as described in Section~\ref{impl-comps-exit} and the result and parameter 
...
The filter \code{gencot-parmodtmpl} is used to generate the Json template for manually filling in the information 
about parameters which may be modified in a function. The filter processes all \code{DeclEvent}s which are function
