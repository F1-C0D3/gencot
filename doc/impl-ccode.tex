...
\subsubsection{Translating C Expressions}
...
As variable for the expression value Gencot uses variables named \code{v<n>'}. Due to the prime character this is
...
Every binding in the main list binds in \code{v1..} atleast the side effect targets bound in \code{v1..} of the previous binding. Thus 
the variables \code{v1..} in the last binding are all side effect targets bound in the main list. This makes it 
easier to determine the objects affected by an expression. The putback list may also introduce side effect targets. Since 
putback lists are usually short Gencot does not collect side effect targets in their last element. Together, the side effect 
targets of the translated expression can be determined as the union of the side effect targets in the last element of the 
main list and all side effect targets bound in the putback list.
...
\paragraph{Dealing with Subexpressions}
...
are translated to binding sequence pairs using \code{bindExpr}.
...
and concatenates these lists according to the sequencing of the groups, adding the side effect targets \code{v1..} in 
the last binding of a previous list to all bindings in the following list, unless they are rebound there.
...
overlapping side effect targets, the side effect targets are all distinct and the targets in the last binding of a previous list
are added to all bindings in the following lists. As described above, the value variables are constructed so that they are 
distinct for all subexpressions. Together, no variable will be rebound by concatenating the lists. 
...
first are put back last. The side effect targets are transferred in the concatenated putback lists as in the main lists,
so that the set of side effect targets increases monotonically in the putback list.

\paragraph{Accessing Struct Members and Array Elements}
...
Its pattern consists of the single variable bound to the \code{put} expression.
...
tuple pattern immediately after the value variable, before all side effect targets, if present. Unlike the side effect
targets the set of component variables does not increase monotonically in subsequent bindings, a component variable is
only present in the binding pattern if it is actually rebound. The reason is that it is not necessary to determine modifications
to component variables in a binding list, since they are handled automatically (by the \code{put} operations).
...
a \code{take} operation binding (transferring all side effect targets) followed by a binding of the new component variable
...
which is bound to \code{v<n>'}. This variable is used to bind the remaining container value in the \code{take} operation and, in 
...
invalid Cogent code, it is detected and simplified in later stages.
...
\paragraph{Translating Pointer Dereferences}
...
\code{*e} is translated by \code{bindExpr} as \code{e.cont} to a pair of binding lists involving a \code{take} and a \code{put}
...
\paragraph{Translating the Address Operator}
...
\paragraph{Translating Function Calls}
...
\paragraph{Translating lvalue Expressions}
...
\subsubsection{Translating C Statements}
