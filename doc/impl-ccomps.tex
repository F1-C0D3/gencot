...
\item[\code{parmod-extfuns}] generates the list of function identifiers of all externally referenced functions.
...
The processor \code{gencot-extfuns} generates the Cogent abstract function definitions. It parses and analyses
...
it determines for every table the invoked functions as described in Section~\ref{impl-ccode-callgraph}. This must
...
descriptions are generated by \code{parmod-extfuns} as described in Section~\ref{impl-ccomps-parmod}.
...
The filter \code{gencot-exttypes} generates all Cogent type definitions which do not result from
translating a C type or tag definition in the <package>. There are two cases:
\begin{itemize}
...
however, Gencot treats variables as if it would translate them to Cogent code referencing the translated declared type.
...
functions for working with it, the syntactic part types may be needed. For derived function types Gencot automatically
...
As described in Section~\ref{impl-ccomps-exit}, Gencot only translates external functions if they are actually invoked.
Therefore, \code{gencot-exttypes} determines the external invoked functions in the same way. However, to support
...
For the variables a similar analysis is performed if they have a function pointer (array) type, only the invoked
variables are processed. For the variables of other types Gencot simply uses all known
(defined or declared) variables as type carriers, including those from system include files.
...
to the Cogent compilation unit, together with all external invoked functions. It then determines all transitively
...
a \code{DeclEvent} for the containing function, which is processed by the handler. Function declarations are also
ignored, since they represent external functions which are separately determined using the invocation relation. Also,
a function declared in one source file may be defined in another source file of the Cogent compilation unit and thus 
...
as described in Section~\ref{impl-ccomps-exit}. If the associated syntactic type is a typedef name, it is resolved
...
For type names it fully resolves the defining type, as described above.

This is implemented by the same monadic action \code{transGlobals} as the main translation to Cogent described
in Section~\ref{impl-ccomps-main}.
...
\subsubsection{Processor \code{parmod-extfuns}}

The processor \code{parmod-extfuns} is used to generate the parameter modification descriptions intended
to be used by \code{gencot-extfuns} (see Section~\ref{impl-ccomps-exit}) and \code{gencot-exttypes} 
(see Section~\ref{impl-ccomps-exttypes}). Since all these parameter modification descriptions are already
...
\code{gencot-extfuns} and \code{gencot-exttypes}. 
...
descriptions needed by \code{gencot-extfuns} and \code{gencot-exttypes}.

The processor \code{gencot-extfuns} processes declarations of external invoked functions. It needs parameter
modification descriptions for these functions and for their parameters of function (pointer (array)) type.
The descriptions for the parameters are needed by \code{gencot-extfuns} to translate the parameter type as part of the 
generated abstract Cogent function definition. 

Although \code{gencot-extfuns} only processes complete function declarations of non-variadic external functions, 
\code{parmod-extfuns} also lists identifiers for incompletely declared functions and for variadic functions,
...
\code{gencot-extfuns} in Section~\ref{impl-ccomps-exit}.
...
Accordingly, \code{parmod-extfuns} lists for all processed type carriers the function ids of all composite type
...
external invoked functions are already included there, however, those with only primitive types are omitted.
Here we also include them, since \code{gencot-extfuns} processes them. Also included are all external 
variables of function pointer (array) type, since they are processed by \code{gencot-exttypes}.
For these external function pointer (array)s descriptions for parameters are not listed, since
...
For the determined type carriers \code{parmod-extfuns} lists the identifiers of 
...
\items declarations of external function pointer (array)s, if the declared type directly specifies
the function type,
\item members of all composite types defined in system include files, if their type is a function pointer (array) type
...
Note that typedef names in system include files need not specify a function type directly for being listed, since 
external typedef names are fully resolved by \code{gencot-exttypes} and the resulting type is needed to translate 
the type definition.
