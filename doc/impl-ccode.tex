...
Together, Gencot defines the type \code{Origin} for representing the origin information:
\begin{verbatim}
  data Origin = Origin { 
...
a list of C constructs is translated, first the corresponding list of \code{NodeInfo}s is retrieved, then it is 
converted to the corresponding list of \code{Origin}s, and finally every origin value is passed to translation function
...
The first and second step together are implemented by the function
\begin{verbatim}
  listOrigins :: CNode a => [a] -> [Origin]
\end{verbatim}
Class \code{CNode} is defined by language-c with all types for C constructs being instances, it allows to retrieve the 
associated \code{NodeInfo} for every construct.
...
This approach implies that for every C construct it's \code{NodeInfo} is translated to an \code{Origin} in a separate preceding 
step which omits all repeated markers. Every translation function from a C construct to a Cogent construct receives 
the \code{Origin} as an additional argument.
...
The translation of the \code{DeclEvent} sequence is implemented by function
\begin{verbatim}
