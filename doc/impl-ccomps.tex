...
from the \code{const} qualifiers in the item's type. 
...
It is intended as a filter for the processors \code{gencot-externs}, \code{items-externs} and \code{parmod-externs} 
...
if it is not defined in the Cogent compilation unit. Since all \code{.h} files used by the Cogent compilation unit 
...
it may be defined in other \code{.c} files of the package or in C libraries external to the package.
...
call graph. Invocations of sub-items which are the result of another fuction are currently not detected. 
