...
a letter ``U'' is prepended to \code{El} in the form of an additional derivation step. This can happen for struct, union
and array types.
...
Together the rules for mapping function types (and function pointer types through the from/to translation 
functions) are
\begin{verbatim}
  t(t1, ..., tn), t (*)(t1, ..., tn) 
    -> (T1, ..., Tn) -> T
...
A parameter may be modified by a C function locally, e.g., by an assignment to a struct member. But it can 
also be modified by passing it or a part of it as parameter to an invoked function which modifies its corresponding
parameter. Gencot does not automatically detect parameter modification, this must be done manually. To support
specifying parameter modification, Gencot generates a Json template from the C program. It can be used 
to fill in all cases where a parameter is modified locally or is modified depending on a parameter modification
of an invoked function. 

The filled template is then read by Gencot upon translating the C program to Cogent
and is used to determine all cases where a parameter may be modified. Parameters of linear type 
are translated to a parameter with readonly type if they are not modified, otherwise a corresponding
component is added to the result type.
...
that means, it is mapped like type \code{t}, but the unbox operator is omitted.
...
An exception are C pointers to functions. It is assumed that the function code cannot be modified, hence for a C pointer 
to function to have the readonly property, its base type need not be \code{const} qualified.

Gencot tests every C pointer type for this property. If it is valid, the type is translated to a Cogent readonly type,
by applying the bang operator \code{!} to the type after translating it as described in Section~\ref{design-types-pointer}.

In particular, the property is valid for all pointer types where the base type contains no pointers, such as
\code{const char*}.
