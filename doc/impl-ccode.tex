...
maps or even on the \cpode{DeclEvent} lists. However, as described in Section~\ref{impl-ccode-trav}, Gencot also
...
internal linkage, it is only valid in its symbol table and my denote a different object in another symbol table. These
...
combined table. Note that if the C program is correct, the identifier may be defined in atmost one C compilation unit
and thus occurs in at most one of the symbol tables.

The typical case for a toplevel identifier with no linkage is a typedef name. It may occur in two symbol tables, if it is
...
the unique identifier is used as key, as described above. If a tag is present, this corresponds to an identifier with 
no linkage and is treated in the same way.
...
contains atmost one reference to the key. When the symbol tables are combined, atmost one of these references are transferred
...
\end{verbatim}
in module \code{Gencot.Input}. It should be applied to the tables built by \code{readFromFile} for two different
\code{.c} files of the same package. The result can then be processed mainly in the same way as described in 
...
\end{verbatim}
where \code{String} is the type used for storing the original C source file name in the user state and
\code{ParmodMap} is the type for storing the parameter modification description. \code{FTrav} is an
...
description as arguments to initialize the state. The second one leaves the latter two empty. The functions are themselves 
...
where the invoked function is specified as an identifier for a function or function pointer, or by a chain of 
member access operations starting at an identifier for an object of struct or union type.  All other invocation where the invoked function is specified in a different way are ignored and not added to the call graph.
...
invoked function pointer as its direct member.
