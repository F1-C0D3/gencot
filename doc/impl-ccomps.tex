...
(see Section~\ref{impl-parmod-ids}): Functions and global variables of external linkage are specified by \code{*<name>}.
For internal linkage \code{<filename>:} is prepended. Function parameters and local variables are specified by 
\code{<funid>/*<name>} where \code{<funid>} is a function identifier for the containing function 
as described in \ref{impl-parmod-ids}. 
Struct and union members are specified by \code{<tag>.*<name>}. Specifying members of untagged
structs and unions is not supported. The \code{"*"} before the \code{<name>} corresponds to the type hint 
described in \ref{impl-parmod-ids}, here it is always \code{"*"} because every list entry must be of a pointer type.
...
\begin{verbatim}
  gencot-translate <source name> <safepointers file> [<parmod file>]
\end{verbatim}
where the arguments are the name of the original C source file, a file with the safe pointer list (see 
Section~\ref{impl-ccomps-pointer}), and the optional parameter modification descriptions.
...
\begin{verbatim}
  gencot-externs <safepointers file> <varlist file> [<parmod file>]
\end{verbatim}
where the arguments are a file with the safe pointer list (see Section~\ref{impl-ccomps-pointer}), a file with the 
names of external variables to be processed, and the optional parameter modification descriptions.
...
It is invoked as
\begin{verbatim}
  gencot-exttypes <safepointers file> <varlist file> [<parmod file>]
\end{verbatim}
where the arguments are a file with the safe pointer list (see Section~\ref{impl-ccomps-pointer}), a file with the 
names of external variables to be processed, and the optional parameter modification descriptions.
...
\begin{verbatim}
  gencot-dvdtypes <safepointers file> <varlist file> [<parmod file>]
\end{verbatim}
where the arguments are a file with the safe pointer list (see Section~\ref{impl-ccomps-pointer}), a file with the 
names of external variables to be processed, and the optional parameter modification descriptions.
