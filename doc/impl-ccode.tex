...
Although the translation described in Section~\ref{impl-ccode-cstats} is mostly independent of any type
...
expression or pattern from the types of sub components.
...
\subsubsection{Expressions for Constants and Variables}

For constants and identifiers the type information is always directly determined from the C program.
...
For Integer constants the C type is determined using \code{tExpr}, then it is mapped to a Cogent type as described
in Section~\ref{design-types-prim} which results in one of the types \code{U8}, \code{U16}, \code{U32}, or \code{U64}.
