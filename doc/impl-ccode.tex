...
The function \code{withDefTable} is used to insert the existing symbol table into the monadic state before
processing the list of \code{DeclEvent}s. Thus a toplevel Gencot processing function has the general form
\begin{verbatim}
procGlobals :: MonadTrav m => DefTable -> [DeclEvent] -> m [R]
procGlobals table gs = do
    withDefTable $ const ((),table)
    mapM procGlobal gs
\end{verbatim}
where \code{R} is the result type of processing a single \code{DeclEvent} and \code{procGlobal} is the 
corresponding monadic action.
