...
to an arbitrary Cogent type expression (which is done for all function types). A generic type for
...
generate the monomorphic types on its own. It uses abstract types of the form
\begin{verbatim}
  type CArrayOf_<element type name>
\end{verbatim}
If the element type is a (pointer to) function type, an element name is created for it, 
so that the array type has the form
\begin{verbatim}
  type CArrayOf_CFunRet_<result type name>
\end{verbatim}
In all other cases the element type is already represented by a type name in Cogent.
...
\begin{verbatim}
  el[...] -> #CArrayOf_El
\end{verbatim}
where El is the result of mapping the element type \code{el} to a Cogent type name.
...
The abstract type has the form
\begin{verbatim}
  type CPointerTo_<type name>
\end{verbatim}
A pointer to \code{void} is mapped to
\begin{verbatim}
  type CPointerTo_Void
\end{verbatim}
...
array type as pointer to array, which results in a type of the form \code{CArrayOf\_T} where \code{t} is 
mapped to \code{T}.
...
\begin{verbatim}
  ...
  union s * -> Union_s
  (*el)[...] -> CArrayOf_El
  as parameter: el[...] -> CArrayOf_El
  otherwise: t * -> CPointerTo_T
\end{verbatim}
where \code{Struct\_s} and \code{Union\_s} are the names introduced for the struct or union types and \code{El} 
and \code{T} are the Cogent type names to which \code{el} and \code{t} are mapped, respectively.
...
\begin{verbatim}
  ...
  dummy_Array_El: () -> CArrayOf_El
  dummy_Pointer_T: () -> CPointerTo_T
\end{verbatim}
...
for mapped union pointer types. The dummy expression for type \code{CArrayOf\_El} is 
\code{dummy\_Array\_El ()} and for type \code{CPointerTo\_T} it is \code{dummy\_PointerTo\_T ()}.
...
type is mapped to its name omitting the unbox operator. Otherwise, a type name of the form \code{CPointerTo\_T} is used.

Always mapping pointer types derived from typedef names with the help of \code{CPointerTo\_T} would result in
...
\begin{verbatim}
  ...
  mapping: snam * -> CPointerTo_Cogent_snam
\end{verbatim}
where \code{Struct\_s} is the name of the Cogent record type corresponding to \code{struct s}. The problem here
is that \code{snam *} is mapped using \code{CPointerTo\_T}, resolving to the Cogent type \code{CPointerTo\_\#Struct\_s}
