...
It would be nice to use a single generic type for all C array types, instead of generating a new name for every 
...
The common antiquoted C typedef would need to have the form 
\begin{verbatim}
...
specification \code{<size>} (if not specified for a function parameter, see below):
\begin{verbatim}
  el[<size>] -> #A'<size>'_El
  el[] -> #A_El
\end{verbatim}
...
In other places, however, Cogent does not use function pointers. Instead, it uses a C enumaration type where 
every known function has an associated enumeration constant. Whenever a function is bound to a variable, passed 
as a parameter or is invoked, it is represented by this enumeration constant in C, i.e., by an integer value.
...
constant. This has two consequences. First, the function pointer type cannot be mapped to a Cogent function type,
since this will not be binary compatible. Instead, it must be mapped to a Cogent abstract type together with abstract functions which translate between the abstract type and the Cogent function type (needed when invoking 
the function in Cogent).

Second, all functions which are passed to Cogent as a parameter or as a field in a record must be known to
...
Mapping all C function types to Cogent abstract types is the reason why names are generated for them, as
described in Section~\ref{design-names}. The goal is to use the same name for every occurrence of a C 
function type. Thus it is infeasible to generate the name from 
...
The abstract functions for translating are named 
\begin{verbatim}
  from_<name>
  to_<name>
\end{verbatim}
where \code{<name>} is the generated name for the abstract Cogent type. Gencot creates the abstract
function definitions together with the abstract type definition. Hence for example the C type
\begin{verbatim}
  int *()(int, short)
\end{verbatim}
the translated to the definitions
\begin{verbatim}
  type F_'U32'U16'_P_U32
  from_F_'U32'U16'_P_U32: F_'U32'U16'_P_U32 -> ((U32,U16) -> P_U32)
  to_F_'U32'U16'_P_U32: ((U32,U16) -> P_U32) -> F_'U32'U16'_P_U32
\end{verbatim}

\subsubsection{Mapping FunctionParameters}

All function pointers are represented by an integer in Cogent, hence a C function pointer type can be 
mapped (through the from/to translation functions) to an arbitrary Cogent function type.
...
Together the rules for mapping function types (through the from/to translation functions) are
\begin{verbatim}
  t(t1, ..., tn), t (*)(t1, ..., tn) 
    -> (T1, ..., Tn) -> T
  t(void), t (*)(void) 
...
abstract function from the unit type to every abstract Cogent type \code{FT} for a C function type:
\begin{verbatim}
  dummy_FT: () -> FT
\end{verbatim}

Then the dummy expression for type \code{FT} is \code{dummy\_FT ()}. 
...
A pointer type \code{t*} to a struct, union, or array type is mapped to the corresponding boxed type, 
that means, it is
...
to an abstract type. For the same reason as for arrays, Gencot cannot use a single generic abstract type for all 
such pointer types. Instead, it creates a monomorphic abstract type for every pointer type used in the program.
As described in Section~\ref{design-names} the abstract type has the form
\begin{verbatim}
  type P_<type name>
\end{verbatim}
A pointer to \code{void} is mapped to
\begin{verbatim}
  type P_Void
\end{verbatim}
in all other cases the target type is already represented by a type name in Cogent.
...
type \code{t*} with the semantics of being a pointer to array of \code{t}. In this case Gencot also 
adjusts the type and maps the 
array type as pointer to the element type.
...
and \code{T} are the Cogent type names to which \code{el} and \code{t} are mapped, respectively. If \code{El} 
...
for mapped union pointer types. The dummy expression for type \code{A\_El} is 
\code{dummy\_A\_El ()} and for type \code{P\_T} it is \code{dummy\_P\_T ()}.
...
the kind of base type, as described above. If the base type is a struct, union, function, or array type, 
the pointer type is mapped to its name omitting the unbox operator. Otherwise, a type name of the form 
\code{P\_T} is used.
...
Therefore Gencot treats every typedef name resolving to a struct, union, function, or array type as if 
it would resolve
...
the base type is a struct, union, function, or array type. In this case a typedef name for the base type is mapped
to a type name for the corresponding boxed type in Cogent. A name for the derived pointer type is mapped
in the same way. 

If the base type in a typedef is another typedef name which eventually resolves to a struct, union, function, or 
