...
\end{verbatim}
where \code{l1} is the type of the first parameter of linear type which is not read-only or has the Discard property, \code{L1}
is its translation, and \code{L2,...,Lm} are the translations of all other such parameter types. The parameter type of
...
If both properties are declared together for the same function the heap is added as last components \code{Tn} and
...
having a C pointer type for which \code{MayNull} is omitted. This list is called the ``Gencot safe pointer list''. It must be 
specified manually by the developer. If a C pointer type is specified in the list \code{MayNull} is omitted for all entities 
...
same form, it applies \code{MayNull} if not prevented by the Gencot safe pointer list.
...
and \code{T} are the Cogent types to which \code{el} and \code{t} are mapped, respectively. When specified in
the Gencot safe pointer list the generic type \code{MayNull} is omitted.
...
form \code{MayNull P} (see Section~\ref{design-types-pointer}). Since it may depend on the context where the type
is used, whether its values may be \code{NULL} or not, it may be manually replaced by type \code{P} in some context.
To be able to do this also when a typedef name is defined for it, Gencot defines the typedef name as a synonym 
for the type \code{P} and translates its use by applying \code{MayNull} to the type synonym.
