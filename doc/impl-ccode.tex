...
For a \code{return} statement the expression is translated to a binding list pair by \code{bindExpr}. If the expression has been
omitted a binding list pair with a single binding
\begin{verbatim}
  v' = ()
\end{verbatim}
in its main list and an empty putback list is generated instead. Then the resulting binding list pair is combined by \code{cmbBinds}
to a binding \code{(v<n>',v1..) = expr} and the translation of the return statement
is the binding
\begin{verbatim}
  (c',v1..) = let (v<n>',v1..) = expr
...
An object declaration introduces one or more names for C objects. The names must not yet be defined in the surrounding
compound statement and they shadow existing names of outer surrounding compound statements. 

Since the scope of a local declaration consists of the statements after it in the surrounding compound statement this is
an issue for the translation of compound statements. Otherwise, a local object declaration is translated to a binding
in the same way as a statement, with two exceptions. First, the side effect targets of the binding
are exactly the variables introduced by the object declaration (either for the value of a translated initialzer or
to a default value, as described in Section~\ref{design-cstats-general}. No control variable is bound, since a declaration
cannot contain (jump) statements. Second, the initializers for struct and array
...
An empty compound statement is translated like a null statement to a
binding
\begin{verbatim}
  c' = (False,False,None)
\end{verbatim}
...
A statement \code{s1} followed by a compound statement \code{s2} is translated as follows. Both statements are translated 
to bindings \code{(c',x1..) = expr1} and \code{(c',y1..) = expr2}. Let \code{z1..} be the union of the side effect targets 
\code{x1..} and \code{y1..} in some order. The compound statement is translated to the binding
\begin{verbatim}
  (c',z1..) = let ((cc',cb',res'),x1..) = expr1
  in if cc' 
     then ((cc',cb',res'),z1..)
     else let (c',x2..) = expr2 in (c',z1..)
\end{verbatim}
