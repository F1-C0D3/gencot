...
\end{verbatim}
The first two are used for constructing and processing the JSON representation. Type \code{Parmod} represents a single
...
The type \code{ParmodMap} is used to represent the information from an evaluated parameter modification description for 
...
type and for every definition of a compound struct or union type it translates all members with function pointer type.
...
\end{verbatim}
The function \code{getRequired} retrieves the function identifier of all invocations on which a parameter depends.
The function \code{evaluateParmods} evaluates a parameter modification description as described in 
Section~\ref{impl-parmod-eval}. The function \code{convertParmods} converts the JSON representation to the compact
form \code{ParmodMap} for applying it when generating Cogent code.
