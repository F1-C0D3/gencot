...
Let \code{z1..} be the union of \code{v1..} and \code{y1..}. Let \code{u1..} be \code{z1..} without \code{v}. Then the
combined binding is
\begin{verbatim}
  (c',z1..) = 
    let (v<n>',v1..) = expr1
...
correctly applied to a shadowed instance of \code{v}. Otherwise \code{u1..} and \code{z1..} are the same and the binding will be
simplified in later stages to the binding
\begin{verbatim}
  (c',z1..) = 
    let (v<n>',v1..) = expr1
    and v = v<n>'
...
Let \code{g1 ... gp} the sequence of these compound statements and let \code{lbl1 ... lblp} be the corresponding labels in the \code{case}
...
\end{verbatim}
where \code{y1..} is the union of \code{v1..} and \code{x1..}. Note that the condition uses \code{cb'} (\code{break} or \code{return}) 
instead of \code{cc'} (\code{break}, \code{return}, or \code{continue}) to correctly translate the semantics of \code{continue} 
...
The expression \code{e2} is translated to a binding \code{v<n>' = expr2} by applying \code{bindExpr} to it and combining the resulting binding
list pair by \code{cmbBinds} (no side effect targets are present because for the translation \code{e2} may not have side effects).
...
\end{verbatim}
where \code{w1..} are all variables occurring free in \code{exprloop} or \code{expr2} which are not part of \code{z1..}, in some order.
The expression \code{exprmax} for the upper limit of number of iterations is constructed as described below.

If \code{c1} is an expression it is translated to a binding \code{(v<n>',u1..) = expr1} by applying \code{bindExpr} to it and 
...
conjunction which contains such a relation. No side effect target of the body statement \code{s} may occur free in \code{evl}
and \code{v} must not be a side effect target of \code{s}.
