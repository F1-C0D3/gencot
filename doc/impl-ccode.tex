...
are generated with internal linkage (specified as \code{static}), so they are removed before combining the symbol tables. 
Alternatively the dummy declarations could have been explicitly specified as \code{extern}, then they are classified 
as declarations by language-c and no error is signaled by Gencot. Note that (manually created) dummy declarations for
