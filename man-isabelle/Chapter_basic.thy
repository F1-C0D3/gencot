...
text \<open>
Isabelle is freely available from \<^verbatim>\<open>https://isabelle.in.tum.de/\<close> for Windows, Mac, and Linux. It is 
actively maintained, there is usually one release every year. Older releases are available in a 
...
text_raw\<open>\label{basic-jedit}\<close>

text\<open>
After invoking Isabelle as editor (see Section~\ref{basic-invoke-edit}) it supports interactive
work with theories.
...
text_raw\<open>\label{basic-jedit-main}\<close>
...
the theory text up to the current cursor position. This includes processing the content
of all imported theory files, if the cursor is after the import statement.

Whenever the cursor is moved, the processing is continued or set back, if ``Continuous
checking'' has not been disabled in the Theories panel.
...
text_raw\<open>\label{basic-jedit-output}\<close>
...
text_raw\<open>\label{basic-jedit-state}\<close>
...
Section~\ref{basic-proof-proc}), and describes what remains to be proven by the
rest of the proof.
...
text_raw\<open>\label{basic-jedit-symbols}\<close>
...
text_raw\<open>\label{basic-jedit-documentation}\<close>

text\<open>
A comprehensive set of documentation documents about Isabelle can be opened 
...
text_raw\<open>\label{basic-jedit-query}\<close>
...
text_raw\<open>\label{basic-jedit-theories}\<close>
...
independently of the cursor position in the text area.
\<close>
...
This text is intended for additional explanations of the formal theory content. It is displayed
in the session document together with the formal theory content.
...
must be delimited by quotes in the source text.
...
An abbreviation definition introduces the name only as a syntactical item which is not known by the internal
logic system, upon input it is automatically expanded, and upon output it is used whenever a term
matches its specification and the term is not too complex. An abbreviation definition is denoted in a similar 
...
In both cases the defined constant may not occur in the \<open>term\<close>, i.e., these forms of definition
...
The \<open>term\<^sub>i\<close> need not be single constants. If an arbitrary term is specified, the constant
\<open>name\<close> becomes an abbreviation for that term. A possible use case is to specify a partial
application as term such as in
@{theory_text[display]
\<open>adhoc_overloading incby5 "((op +) 5)"\<close>}
...
@{text[display]
\<open>\<And> x\<^sub>1 \<dots> x\<^sub>n. \<lbrakk>A\<^sub>1; \<dots>; A\<^sub>n\<rbrakk> \<Longrightarrow> C\<close>}
...
\<open>\<And> x\<^sub>1 \<dots> x\<^sub>n. \<lbrakk>A\<^sub>1; \<dots>; A\<^sub>n\<rbrakk> \<Longrightarrow> C\<close> is the ``structured'' proposition 
@{theory_text[display]
\<open>C if A\<^sub>1 \<dots> A\<^sub>n for x\<^sub>1 \<dots> x\<^sub>n\<close>}
Again, types can be specified for some variables in the usual form. The assumptions and the variables
may be grouped or separated for better readability by the keyword \<^theory_text>\<open>and\<close>.
...
\<open>C, A\<^sub>1, \<dots>, A\<^sub>n\<close> must be individually quoted. The \<open>x\<^sub>1, \<dots>, x\<^sub>n\<close> need not be quoted, but if a
...
are displayed in the Output panel (see Section~\ref{basic-jedit-output}).
\<close>
...
A theorem \<^theory_text>\<open>theorem "\<And> x\<^sub>1 \<dots> x\<^sub>n. \<lbrakk>A\<^sub>1; \<dots>; A\<^sub>n\<rbrakk> \<Longrightarrow> C" \<proof>\<close> can also be specified in the form
@{theory_text[display]
\<open>theorem 
...
be grouped by \<^theory_text>\<open>and\<close> and types can be specified for some or all of them. The content between
...
panel and optionally also in the Output panel (see Sections~\ref{basic-jedit-output} 
and~\ref{basic-jedit-state}), according to the cursor position in the proof text.
\<close>
...
As usual, the assumptions can be named and grouped by \<^theory_text>\<open>and\<close>.
...
\<open>theorem 
  ...
  shows C
  proof \<open>method\<close>
    from name\<^sub>1 have F\<^sub>1 \<proof>
    \<dots>\<close>}

Additionally, the set of all assumption specified in this form of a theorem is automatically named 
...
context by referring them by name.
\<close>
...
As usual, types can be specified for (some of) the variables and the variables can be grouped by \<^theory_text>\<open>and\<close>. 
...
where \<open>pattern\<close> is a term which may contain unbound unknows. The \<^theory_text>\<open>let\<close> statement unifies \<open>pattern\<close>
and \<open>term\<close>, i.e., it determines terms to substitute the unknowns, so that the pattern becomes
syntactically equal to \<open>term\<close>. If that is not possible, an error is signaled, otherwise the
unknowns are bound to the substituting terms.
...
The pattern and the term may contain unknowns which are already bound. They are substituted 
by their bound terms before the pattern matching is performed. Thus the term and the pattern
...
three-dot-symbol available for input in the Symbols panel (see ~Section~\ref{basic-jedit-symbols})
...
Section~\ref{basic-jedit-output}).
...
specifications \<open>simp add:\<close>, \<open>simp del:\<close>, and \<open>simp only:\<close>. For example, additional
...
text \<open>
\<^item> splitting goals, solving each one in a separate context
\<^item> named contexts, case statement
\<^item> case based reasoning
\<^item> cases rules
\<^item> cases method
\<^item> induction
\<^item> induction rules
\<^item> induction method
\<close>
*)
end
