...
To use the call graph the \code{CallGraph} module defines an extension \code{CTrav} of the \code{FTrav} monad
(see Section~\ref{impl-ccode-trav}) with the call graph in the user state. The corresponding execution function is
\begin{verbatim}
  runWithCallGraph :: CallGraph -> DefTable -> String -> CTrav a -> IO a
\end{verbatim}
which extends \code{runWithTable} by passing the call graph. The action to access the call graph is
\begin{verbatim}
