...
After determining the invoked functions, the tables are combined and the invocations are reduced to those for
which a declarations is present in the combined table. These are the invocations of functions external to all
read C souurces. The processor translates their declarations using the function \code{transGlobals} of
...
The processor \code{parmod-extfuns} generates a list of identifiers for all invoked external functions.
...
The processor is implemented in the same way as \code{gencot-extfuns}, determining the declarations of all invoked
